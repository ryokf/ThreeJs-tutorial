...
// Cuboid Collider
const cubeSize = 1
const cubeMesh = new THREE.Mesh(new THREE.BoxGeometry(cubeSize, cubeSize * 2, cubeSize), new THREE.MeshNormalMaterial())
cubeMesh.castShadow = true
scene.add(cubeMesh)
const cubeBody = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).setCanSleep(false))
const cubeShape = RAPIER.ColliderDesc.cuboid(cubeSize / 2, cubeSize / 2 * 2, cubeSize / 2).setMass(1).setRestitution(1)
...
// // Cylinder Collider
const cylinderMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 1, 2, 1024), new THREE.MeshNormalMaterial())
...
renderer.domElement.addEventListener('click', (e) => {
  ...
  if (intersects.length) {
    dynamicBodies.forEach((b) => {
      b[0] === intersects[0].object && b[1].applyImpulse(new RAPIER.Vector3(5, 1, 0), true)
    })
  }
})
